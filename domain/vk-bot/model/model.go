...
func (c Command) Message() string {
	switch c {
	case Authorization:
		return `Здравствуйте! Для продолжения работы с ботом требуется ввести ваш логин и пароль, используемые для системы Zammad.

ℹ Сейчас введите логин:`
	case Home:
		return `ℹ При возникновении обращения просто напишите его мне.`
	case Password:
		return `ℹ Теперь введите пароль:`
	case DeleteAuth:
		return `Вы вышли из системы! Для продолжения работы с ботом требуется ввести ваш логин и пароль, используемые для системы Zammad.

ℹ Сейчас введите логин:`
	case CancelAuth:
		return `Вы отменили авторизацию! Для продолжения работы с ботом требуется ввести ваш логин и пароль, используемые для системы Zammad.
...
ℹ Сейчас введите логин:`
	...
	case ChangeSubject:
		return "➕ Введите тему:\n"
	case ChangeOwner:
		return "➕ Выберите ответственное лицо:\n"
	case SendMessage:
